%recreate surface elevation time series, based on multiple wave groups. 

clc;clear all; fclose all

[date, bw, E, f]=specsum('./NOAA_inputs2/DATA_spec.txt',1,1);
[date, b, A1, f]=specsum('./NOAA_inputs2/DATA_dir1.txt',1,0);
[date, b, A2, f]=specsum('./NOAA_inputs2/DATA_dir2.txt',1,0);
[date, b, R1, f]=specsum('./NOAA_inputs2/DATA_r1.txt',1,0);
[date, b, R2, f]=specsum('./NOAA_inputs2/DATA_r2.txt',1,0);

%H=E./bw./1030./9.8*3.28084^2;
H=sqrt(4*E./(bw.*(1030*9.8)));

Rot=89;
len=2500;
numpts=100;
%project the boundary to find x and y locations
xlen=len*sind(Rot);
ylen=len*cosd(Rot);
%initialize spatial grid
x=linspace(0,xlen,numpts);
y=linspace(0,ylen,numpts);
%identity matris is the grid point locations
I=eye(numpts);
boundary=find(I==1);

%time inputs
dt=1;
ttot=200;

% figure
% subplot(3,1,1)
% plot(H)
% subplot(3,1,2)
% plot(bw)
% subplot(3,1,3)
% plot(f)

times=0:dt:ttot;
sig=2*pi.*f;

k=4*pi^2.*f.^2./9.8;

% 
% eta=zeros(length(f),10000);
% 
% for i=1:1:length(f)
% 
% eta(i,:)=H(i)./2.*cos(-sig(i).*t);
% 
% end
% 
% eta2=ones(1,length(t));
% 
% for i=1:1:length(f)
% 
% eta2(:)=eta2+eta(i,:);
% 
% end
% figure
% plot(eta2)

%find direction
Dir=((A1(1,:)+A2(1,:))./2);
nums=find(Dir ~= -999);

[xx yy]=meshgrid(x,y);

%solve for L, and it's x- and y- projections
L=9.8./(2.*pi.*f.^2);
Lx=2.*L.*sind(Dir);     %x-projection of wavelength
Ly=2.*L.*sind(90-Dir);  %y-projection

zzx=find(Lx==0);
zzy=find(Ly==0);

%convert from wavelength to wavenumber, and sigma
kx=2.*pi./Lx;
ky=2.*pi./Ly;

kx(zzx)=0;
ky(zzy)=0;

sig=(2*pi).*f;

%define t and number of frequancies
%freqs=[linspace(0.1,nums(length(nums)),.5*length(times)),nums(length(nums))*ones(1,length(times))];
freqs=linspace(0.1,nums(length(nums)));
it=1;


%subplot(1,2,2)
% writerObj = VideoWriter('FileName.avi');
% set(writerObj,'FrameRate',20) % ~25 is normal playback speed (frames per sec)
% open(writerObj);
%  
% set(gca,'nextplot','replacechildren');
% set(gcf,'Renderer','zbuffer');

rst=randn*100

for t=1:length(times)
    
    hold off
    FF=ceil(freqs(it));
    FF=length(nums-3);
for i=1:FF

    eta2d(:,:,i)=H(i)/2.*(cos(kx(i)*xx+ky(i)*yy+sig(i).*times(t)));
    
end

%preallocating surface elevations
eta=sum(eta2d,3);

% waterfall(x,y,eta')
pcolor(x,y,eta);zlim([-5 10]);axis equal;shading flat;caxis([-3 3]);hold on
plot(xx(boundary),yy(boundary),'LineWidth',2);
BC(:,t)=eta(boundary);pause(.001)
end

%%
figure
Lbc=linspace(0,len,numpts);
waterfall(Lbc,times(1:t),BC');colormap gray;caxis([0 10000]);view(-25,80)
xlabel('distance(m)')
ylabel('time (s)')

